...
// Added 'backlight' to props
const LcdScreen = ({ row1Data, row2Data, backlight }) => {
  ...
  // The PCB Board remains fully visible.
      ...
      <div className="relative bg-slate-900 px-3 py-2 rounded-md shadow-lg border-t border-slate-700 mx-2 mb-2 z-20">
         ...
         >
            ...
            <div className="flex gap-[2px] md:gap-[3px] relative z-10">
                {r1.map((code, i) => <Cell key={`r1-${i}`} char={String.fromCharCode(code)} />)}
            </div>
            <div className="flex gap-[2px] md:gap-[3px] relative z-10">
                {r2.map((code, i) => <Cell key={`r2-${i}`} char={String.fromCharCode(code)} />)}
            </div>
         </div>
      </div>
